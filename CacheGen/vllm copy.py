...
def encode_function(path_to_original_kv, quantization_config, CHUNK_SIZE, output_path):
    ...
    output_dict[f"start_indices"] =  torch.tensor(start_indices).int().cuda()
...
if __name__ == "__main__":
    for i in tqdm(list(range(11))):
        encode_function(
            f"/workspace/share/cache_layer_mistral/kvcache_regroup/{i}.pt",
            "./config/quantization_7b.json",
            3200,
            f"/workspace/share/cache_layer_mistral/kvcache_compressed/{i}.pkl",
        )
